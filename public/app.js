...
class ExpressionTrainer {
  ...
  async startRecording() {
    ...
    try {
      ...
      this.autoThresholdChars = st?.feedback?.autoThresholdChars || 50;
      this.autoCooldownSec = st?.feedback?.cooldownSec || 8;
    } catch (e) { this.autoThresholdChars = 50; this.autoCooldownSec = 8; }
    // Phase 2: WS 会话 + Recorder 录音
    if (typeof window.api.connectWS !== 'function') {
      this.showError('通信层未加载');
      return;
    }
    ...
    try {
      // 建立 WS + 绑定回调
      window.api.connectWS({
        onPartial: (text) => this.renderSubtitle(text, false),
        onFinal: (msg) => this.handleASRResult({ text: msg.text, isFinal: true, analysis: msg.analysis }),
        onError: (message) => this.showError(`识别错误: ${message}`),
      });

      this.recorder = new Recorder({
        onChunk: (wav) => window.api.wsSendAudio(wav),
        onError: (e) => this.showError(`录音错误: ${e.message}`),
      });
      await this.recorder.start({ chunkMs: 1000 });

      // 通知后端会话开始（发送 start 消息，建立 FunASR 会话）
      window.api.wsSendStart();
    } catch (err) {
  ...
  async stopRecording() {
    ...
    if (typeof window.api.wsSendStop === 'function') window.api.wsSendStop();
    if (typeof window.api.stopASR === 'function') await window.api.stopASR();
    this.isRecording = false;
    this.isPaused = false;
  ...
  handleASRResult({ text, isFinal, analysis }) {
    if (isFinal) {
      this.sentences.push(text);
      this.fullText += text;
      // 优先使用后端随 final 带回来的 analysis（避免重复请求）；否则回退本地分析
      if (analysis) {
      ...
      // 每阈值触发一次AI反馈（阈值/冷却从设置读取，默认 50 字 / 8s）
      const threshold = this.autoThresholdChars || 50;
      const cooldownMs = (this.autoCooldownSec || 8) * 1000;
      const sinceLast = Date.now() - (this.lastFeedbackAt || 0);
      if (this.fullText.length - this.lastFeedbackText.length >= threshold && sinceLast >= cooldownMs) {
